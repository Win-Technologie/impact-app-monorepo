...
export default function SettingsOptions({ currentLanguage, appVersion }) {
    ...
    return (
        <>
            ...
            <View style={styles.section}>
                <View style={styles.sectionTitle}>
                    <Text style={styles.sectionTitleText}>{t('account.informationandother')}</Text>
                </View>
                {renderSettingOption(t('account.helpandfaq') , "help-with-circle", Entypo,()=>handlePressAide())}
                {renderSettingOption(t('account.about'), "info-circle", FontAwesome,()=>handlePressAPropos())}
                {renderSettingOption(t('account.conditionandregulation'), "balance-scale-left", FontAwesome5,()=>handlePressConditions())}
                {renderSettingOptionWithDetail(t('account.updateandreinstallation'), "cycle", Entypo, appVersion,()=>handlePressMisesAJour())}
            </View>
        </>
    );
