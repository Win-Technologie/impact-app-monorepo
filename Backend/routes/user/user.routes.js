...
// Utilise les middlewares d'authentification et de vérification de session active
router.post('/code/generate', [userAuth.ensureAuth, userAuth.isActiveSession], userController.generateQRCode);

// Route pour obtenir l'ID utilisateur et envoyer toutes les informations utilisateur
// Utilise les middlewares d'authentification et de vérification de session active
router.post('/code/read', [userAuth.ensureAuth, userAuth.isActiveSession], userController.readAndSendUserInfo);

// Route pour renvoyer un code de vérification
// Utilise les middlewares d'authentification et de vérification de session active
router.post('/code/resend', [userAuth.ensureAuth, userAuth.isActiveSession], userController.resendVerificationCode);

/* ENCRYPT DATA TESTING ENDPOINTS */

// Route pour tester la sécurité des données chiffrées
// Utilise les middlewares d'authentification et de vérification de session active
router.post('/test/data/encrypt', [userAuth.ensureAuth, userAuth.isActiveSession], userController.encryptMyData);

// Route pour obtenir les informations complètes du véhicule de l'utilisateur (auto, assurance)
// Utilise les middlewares d'authentification et de vérification de session active
router.get('/user/vehicle/info/:vehicleId', [userAuth.ensureAuth, userAuth.isActiveSession], userController.getMyAutoFullInfo);

// Route pour valider l'inscription d'un utilisateur
// Utilise les middlewares d'authentification et de vérification de session active
router.post('/user/validate/:id?', [userAuth.ensureAuth, userAuth.isActiveSession], userController.validateInscription);
