...
async function sendExpirationDriverLicensesNotifications() {
    try {
        ...
        const promises = notificationDates.map(async (notificationDate) => {
            // Récupérer les véhicules à notifier pour la date donnée
            const driverLicensesToNotify = await getDriverLicensesToNotify(notificationDate);
            // Vérifier si la date de notification est expirée ou non
            const isExpired = notificationDate < currentDate;
            // Traiter les véhicules à notifier et retourner le nombre de notifications envoyées
            const notificationsSent = await processDriverLicenses(driverLicensesToNotify, isExpired);
            return notificationsSent;
        });
...
async function getDriverLicensesToNotify(notificationDate) {
    try {
        ...
        // console.log(isExpired);
        // Construire la requête de recherche en fonction de la date de notification
        const query = isExpired ? { 'expires': { $lte: notificationDate } } : { 'expires': notificationDate };
        // Récupérer les driver licenses à notifier
        const driverLicensesToNotify = await drivingLicensesCollection.find(query, {
            projection: {
                user: 1,
                number: 1,
                expires:1
    ...
    try {
        ...
        for (const lincese of driverLicensesToNotify) {
            ...
            if (user && user.email) {
                ...
                const subject = isExpired ? "Your driving license has expired!" : "Your driving license is about to expire!";
                const expirationDate = lincese.expires;
                console.log(expirationDate);
                const msg = isExpired ? `Hello ${user.name}, your driving license with number ${lincese.number} has an expiration date of ${expirationDate} and has already expired. Please take the necessary actions.` :
