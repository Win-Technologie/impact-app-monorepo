...
async function modifAndGetUserVeriffAttributes(
  verificationId,
  status,
  verifStatus,
  verifAproved,
  ...
  try {
    ...
    if (updateResult.modifiedCount === 0) {
      ...
      response.msg =
        "Modif User Veriff Attributes: Failed to update user: Failed to update user";
    }
...
// OK
async function uploadAllImagesToVeriff(
  sessionId,
  frontBase64,
  backBase64,
  selfieBase64,
  ...
  try {
    ...
    const signatureFront = generateHMACSignature(
      payloadImageFront,
      privateApiKey,
    );
    const signatureBack = generateHMACSignature(
      payloadImageBack,
      privateApiKey,
    );
    const signatureFace = generateHMACSignature(
      payloadImageFace,
      privateApiKey,
    );
    const signatureCompleted = generateHMACSignature(
      payloadUploadCompleted,
      privateApiKey,
    );
    ...
    const resRecto = await axios.post(`${urlMedia}/media`, payloadImageFront, {
      headers: headers(signatureFront),
    });
    //    console.log("************************");
    //    console.log("resRecto");
    //    console.log(resRecto);

    const resVerso = await axios.post(`${urlMedia}/media`, payloadImageBack, {
      headers: headers(signatureBack),
    });
    //    console.log("************************");
    //    console.log("resRecto");
    //    console.log(resVerso);

    const resSelfie = await axios.post(`${urlMedia}/media`, payloadImageFace, {
      headers: headers(signatureFace),
    });
    ...
    const response = await axios.patch(urlMedia, payloadUploadCompleted, {
      headers: headers(signatureCompleted),
    });
