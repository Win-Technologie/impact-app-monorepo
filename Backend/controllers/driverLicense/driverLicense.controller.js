...
async function validateLicenseData(req) {
  await Promise.all([
    ...
    body("photoRecto")
      ...
      .isString()
      .withMessage(
        "La photo <<recto>> droite est obligatoire et doit être une chaîne de caractères",
      )
      .run(req),
    body("photoVerso")
      .notEmpty()
      .isString()
      .withMessage(
        "La photo <<verso>> droite est obligatoire et doit être une chaîne de caractères",
      )
      .run(req),
    body("photoSelfie")
      .notEmpty()
      .isString()
      .withMessage(
        "La photo <<selfie>> droite est obligatoire et doit être une chaîne de caractères",
      )
      .run(req),
...
async function validateUpdateLicenseData(req) {
  try {
    ...
    return res
      .status(500)
      .json({ msg: "GET DL : Erreur de serveur interne", error: error });
  }
}
...
async function GetMyLicense(req, res) {
  try {
    ...
    const myLicense = await drivingLicensesCollection.findOne({
...
async function UpdateDrivingLicense(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({ msg: "UPDATE DL : Erreur de serveur interne", error: error });
  }
}
...
async function DeleteDrivingLicense(req, res) {
  try {
    ...
    const findLicensePromise = drivingLicensesCollection
      .findOne({ _id: licenseId })
      ...
      .then((myLicense) => {
        ...
        if (!myLicense) {
          ...
          return res
            .status(404)
            .json({ msg: "Aucune licence n'a été trouvée" });
        }
    ...
    const deleteLicensePromise = drivingLicensesCollection.deleteOne({
      _id: licenseId,
    });

    // Exécution des promesses en parallèle
    const [userId, deleteResult] = await Promise.all([
      findLicensePromise,
      deleteLicensePromise,
    ]);
    ...
    if (!responseSent && deleteResult.deletedCount > 0) {
      ...
      return res
        .status(200)
        .json({ msg: "Licence de conduire supprimée avec succès" });
    } else if (!responseSent) {
      return res
        .status(400)
        .json({ msg: "La licence n'a pas pu être effacée" });
    }
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({ msg: "DELETE DL : Erreur de serveur interne", error: error });
  }
}
...
const createDrivingLicence = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({
        msg: "Permis de conduire créé avec succès",
