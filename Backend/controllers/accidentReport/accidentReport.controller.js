...
const USER_ROUTER_IMG_ACCIDENT_REPPORT_PATH =
  process.env.USER_ROUTER_IMG_ACCIDENT_REPPORT_PATH;
...
async function validateAccidentReport(req) {
  await Promise.all([
    ...
    body("vehicle.brand")
      .isString()
      .isLength({ min: 3 })
      .withMessage("Vehicle brand must be at least 3 characters long")
      .run(req),
    body("vehicle.model")
      .isString()
      .isLength({ min: 3 })
    ...
    body("vehicle.plate")
      ...
      .run(req),
    body("vehicle.serialNumber")
      .isString()
      .isLength({ min: 3 })
      .withMessage("Vehicle serial number must be at least 3 characters long")
      .run(req),
    body("vehicle.owner")
      .isString()
      .isLength({ min: 3 })
      .withMessage("Vehicle owner must be at least 3 characters long")
      .run(req),
    body("vehicle.documents")
      .isArray()
      .withMessage("Vehicle documents must be an array")
      .run(req),
    body("vehicle.isActive")
      .isBoolean()
      .withMessage("Vehicle isActive must be a boolean")
      .run(req),
    body("vehicle.dateAdded")
      .isISO8601()
      .toDate()
      .withMessage("Invalid date added")
      .run(req),

    // Validate vehicle immatriculation details
    body("vehicle.immatriculation.numeroCertificatImmatriculation")
      .isString()
      .isLength({ min: 3 })
    ...
    body("vehicle.immatriculation.numeroEssieux")
      .isInt({ min: 1 })
      .withMessage("Number of axles must be a positive integer")
      .run(req),
    body("vehicle.immatriculation.masseNette")
      .isInt({ min: 1 })
      .withMessage("Net weight must be a positive integer")
      .run(req),
    body("vehicle.immatriculation.cylindree")
      .isInt({ min: 1 })
      .withMessage("Engine displacement must be a positive integer")
      .run(req),
    body("vehicle.immatriculation.numeroDossier")
      .isString()
      .isLength({ min: 3 })
      .withMessage("Dossier number must be at least 3 characters long")
      .run(req),
    body("vehicle.immatriculation.categorieUsage")
      .isString()
      .isLength({ min: 3 })
      .withMessage("Usage category must be at least 3 characters long")
      .run(req),

    // Validate driver license details
    body("driverLicense.number")
      .isString()
      .isLength({ min: 8 })
      .withMessage("License number must be at least 8 characters long")
      .run(req),
    body("driverLicense.name")
      .isString()
      .isLength({ min: 3 })
      .withMessage("Driver name must be at least 3 characters long")
      .run(req),
    body("driverLicense.lastName")
      .isString()
      .isLength({ min: 3 })
      .withMessage("Driver last name must be at least 3 characters long")
      .run(req),
    body("driverLicense.birthdate")
      .isISO8601()
      .withMessage("Invalid birthdate")
      .run(req),
    body("driverLicense.address")
      .isString()
      .isLength({ min: 3 })
      .withMessage("Driver address must be at least 3 characters long")
      .run(req),
    body("driverLicense.appartment")
      .optional()
      .isString()
      .isLength({ min: 2 })
      .withMessage("Apartment must be at least 2 characters long")
      .run(req),
    body("driverLicense.country")
      .isString()
      .isLength({ min: 3 })
      .withMessage("Country must be at least 3 characters long")
      .run(req),
    body("driverLicense.province")
      .isString()
      .isLength({ min: 2 })
      .withMessage("Province must be at least 2 characters long")
      .run(req),
    body("driverLicense.postalCode")
      .isString()
      .isLength({ min: 4 })
      .withMessage("Postal code must be at least 4 characters long")
      .run(req),
    body("driverLicense.licenseClass")
      .isString()
      .isLength({ min: 1 })
      .withMessage("License class must be at least 1 character long")
      .run(req),
    body("driverLicense.sex")
      .isString()
      .isLength({ min: 1 })
      .withMessage("Sex must be at least 1 character long")
      .custom((value) => ["M", "F"].includes(value))
      .withMessage('Sex must be "M" or "F"')
      .run(req),
    body("driverLicense.rest")
      .optional()
      .isString()
      .isLength({ min: 2 })
      .withMessage("Rest must be at least 2 characters long")
      .run(req),
    body("driverLicense.mention")
      .optional()
      .isString()
      .isLength({ min: 2 })
      .withMessage("Mention must be at least 2 characters long")
      .run(req),
    body("driverLicense.height")
      .optional()
      .isString()
      .isLength({ min: 2 })
      .withMessage("Height must be at least 2 characters long")
      .run(req),
    body("driverLicense.weight")
      .optional()
      .isString()
      .isLength({ min: 2 })
      .withMessage("Weight must be at least 2 characters long")
      .run(req),
    body("driverLicense.issued")
      .isISO8601()
      .toDate()
      .withMessage("Invalid issuance date")
      .run(req),
    body("driverLicense.expires")
      .isISO8601()
      .toDate()
        ...
        if (insurance) {
          return [
            body("insurance.provider")
              .isString()
              .isLength({ min: 3 })
              .withMessage("Provider must be at least 3 characters long")
              .run(req),
...
async function newAccidentReport(req, res) {
  try {
    ...
    const _accidentSketch =
      accidentDataArray[0].accidentSketch || "not provided";
    const _accitendType = accidentDataArray[0].accitendType || "not provided";
    const _vehicleDamageDescription =
      accidentDataArray[0].vehicleDamageDescription || "not provided";
    ...
    if (!photos || photos.length < 3 || photos.length > 6) {
      return res
        .status(400)
        .json({ msg: "Limite de photos : minimum 3 et maximum 6." });
    }
    ...
    const photoPaths = photos.map((photo) => {
      ...
      const filePath = path.join(
        USER_ROUTER_IMG_ACCIDENT_REPPORT_PATH,
        uniqueFileName,
      );
    ...
    const vehicleReports = accidentDataArray.map((accidentData) => {
      ...
      const dateDelivranceFormat = new Date(
        vehicle.immatriculation.dateDelivrance,
      );
    ...
    return res
      .status(201)
      .json({
        msg: "Nouveau rapport d'accident créé avec succès",
        accidentId: newAccidentReport._id,
        newAccidentReport,
      });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({
        msg: "Erreur interne du serveur lors de la création du rapport d'accident",
        error: error.message,
      });
  }
}
...
async function joinToAccidentReport(req, res) {
  ...
  const findAccidentReport = await accidentReportCollection.findOne({
  ...
  const vehicleData = instanceVehicleData(
    owner,
    vehicle,
    insurance,
    driverLicense,
  ...
  return res
    .status(statusCode)
    .json({ msg: "Connection to accident report OK" });
}

function instanceVehicleData(
  owner,
  vehicle,
  insurance,
  driverLicense,
...
async function updateAccidentReport(req, res) {
  try {
    ...
    const id =
      userData && userData.user_id ? userData.user_id : myToken.user_id;
    ...
    if (userData.vehicleNumber === "v01") {
        ...
        if (userData.vehicleDamage == "true") {
          if (!userData.vehicleDamageDescription) {
            return res
              .status(400)
              .json({
                msg: "Vous devez introduire une description des dommages",
              });
          }
          myUserData.vehicleADamage = true;
          myUserData.vehicleADamageDescription =
            userData.vehicleDamageDescription;
        } else {
      ...
      if (userData.injured && userData.injured == "true") {
        if (!userData.injuredDescription) {
          return res
            .status(400)
            .json({ msg: "Vous devez fournir une description des lésions" });
        }
      ...
      if (userData.vehicleTowed) {
        ...
        if (
          userData.vehicleTowed !== "true" &&
          userData.vehicleTowed !== "false"
        ) {
          return res
            .status(400)
            .json({
              msg: "le véhicule remorqué doit être <<vrai>> ou <<faux>>",
            });
        }
        ...
        return res
          .status(400)
          .json({
            msg: "Vous devez indiquer si votre véhicule a été remorqué",
          });
      }
      ...
      if (userData.vehicleDamage) {
        if (userData.vehicleDamage == "true") {
          ...
          myUserData.vehicleBDamageDescription =
            userData.vehicleDamageDescription;
        } else {
      ...
      userData.vehicleNumber != "v01" ||
      userData.vehicleNumber != "v02"
    ) {
      return res
        .status(400)
        .json({ msg: "you must indicate vehicle's number" });
    }
    ...
    return res
      .status(500)
      .json({ msg: "GET Accident : Erreur de serveur interne", error: error });
  }
}
...
async function getAccidentReport(req, res) {
  try {
    ...
    const accidentReportCollection = mainDb.collection(
      ACCIDENTREPORTS_COLLECTION,
    );
...
async function getUserAccidentReports(req, res) {
  try {
    ...
    const totalReports = await accidentReportCollection.countDocuments({
...
async function getUserAccidentDetails(req, res) {
  try {
    ...
    if (!accidentReports || accidentReports.length === 0) {
      return res
        .status(404)
        .json({ msg: "Aucun rapport d'accident trouvé pour cet utilisateur" });
    }

    // Préparer les détails à retourner
    const accidentDetails = accidentReports.map((report) => ({
      accidentDate: report.accidentDate,
      profilesPhotos: report.vehicles
        .map((vehicle) => vehicle.personalDetails.profileImagePath)
        .filter((photo) => !!photo),
      vehicleDetails: report.vehicles.map((vehicle) => ({
        vehicleBrand:
          vehicle.vehicleDetails.registrationCertificate.vehicleBrand,
        vehicleModel: vehicle.vehicleDetails.registrationCertificate.year,
        licensePlateNumber:
          vehicle.vehicleDetails.registrationCertificate.licensePlateNumber,
      })),
      userDetails: report.vehicles.find((vehicle) => vehicle.user === userId)
        .personalDetails,
      accidentPhoto: report.accidentSketch || "Photo non fournie",
      accidentDescription:
        report.vehicleDamageDescription || "Description non fournie",
    }));
