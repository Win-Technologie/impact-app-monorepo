...
async function createApplicant(myUser, driverLicense) {
  try {
    ...
    return {
      success: true,
      msg: "Client ONFIDO créé avec succès",
...
async function getAllApplicants(req, res) {
  try {
    ...
    console.error(
      "Erreur lors de la récupération des demandeurs Onfido:",
      error,
    );
    return res
      .status(500)
      .json({
        msg: "Erreur interne du serveur lors de la récupération des demandeurs Onfido",
      });
  }
}
...
async function deleteApplicantByEmail(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({
        msg: "Erreur interne du serveur lors de la suppression du demandeur Onfido",
      });
  }
}
...
async function getApplicantByEmail(req, res) {
  try {
    ...
    if (!existingUser) {
      return res
        .status(404)
        .json({ msg: "l'utilisateur n'existe pas dans la bas de donnée" });
    }
    ...
    for (const applicant of applicants) {
      if (applicant.email === email) {
        // Utilisez la variable 'email' au lieu de 'applicantEmail'
        existingApplicant = applicant;
      }
    }
    ...
    console.error(
      "Erreur lors de la récupération du demandeur par email :",
      error,
    );
    return res
      .status(500)
      .json({
        msg: "Erreur interne du serveur lors de la récupération du demandeur par email",
      });
  }
}
...
async function verifyDrivingLicense(req, res) {
  try {
    ...
    console.error(
      "Erreur lors de la vérification du permis de conduire de l'applicant :",
      error,
    );
    res
      .status(500)
...
async function verifyDrivingLicense02(
  user,
  drivingLicense,
  applicant_id,
  side,
  ...
  try {
    ...
    if (
      drivingLicense.country === "United States" ||
      drivingLicense.country === "united states"
    ) {
      isoCountry = "US";
    }
    ...
    if (!drivingLicenseCheck) {
      ...
      return {
        success: false,
        msg: "Error while crating driving license check",
      };
    }
    console.log("CREATED");
    return {
      success: true,
      msg: "Driver licence is on check",
      drivingLicenseCheck: drivingLicenseCheck,
    };
  } catch (error) {
    console.error(
      "Erreur lors de la vérification du permis de conduire de l'applicant :",
...
async function verifyDrivingLicense(user, drivingLicense, applicant_id, side) {
  try {
    ...
    if (side === "front") {
      ...
      return {
        success: false,
        msg: "Côté invalide du permis de conduire",
        sideFailded: side,
      };
    }
    ...
    console.error(
      "Error durante la verificación del permiso de conducir:",
      error,
    );
    throw error;
  }
