...
} = require("../onfido/onfido.controller");
...
async function validateUpdateRegisterUserFields(req) {
  if (req.body.email) {
    ...
    await body("email")
      .isEmail()
      .withMessage("L'adresse e-mail est requise et doit être valide")
      .matches(/^.+@.+\..+$/)
      .withMessage("L'adresse e-mail est invalide, l'arobase (@) est manquante")
      .run(req);
  }
  if (req.body.name) {
    // Validation du nom
    await body("name")
      .notEmpty()
      .isLength({ min: 2 })
      .withMessage("Le nom est requis et doit contenir au moins 2 caractères.")
      .run(req);
  }
  if (req.body.lastName) {
    ...
    await body("lastName")
      ...
      .isLength({ min: 2 })
      .withMessage(
        "le nom de famille est requis et doit contenir au moins 2 caractères.",
      )
      .run(req);
  }
  ...
  if (req.body.phone) {
    ...
    await body("phone")
      .isNumeric()
      .isLength({ min: 10 })
      .withMessage("Le numéro de téléphone est requis et doit être numérique")
      .run(req);
  }
  if (req.body.address) {
    ...
    await body("address")
      .isLength({ min: 4 })
      .withMessage("L'adresse est requise et doit avoir au moins 4 caractères")
      .run(req);
  }
  if (req.body.postalCode) {
    ...
    await body("postalCode")
      .isLength({ min: 4 })
      .withMessage(
        "Le code postal est requis et doit contenir au moins 4 caractères.",
      )
      .run(req);
  }
  if (req.body.province) {
    ...
    await body("province")
      .isLength({ min: 4 })
      .withMessage(
        "La province est requis et doit contenir au moins 4 caractères.",
      )
      .run(req);
  }
  if (req.body.city) {
    ...
    await body("city")
      .isLength({ min: 4 })
      .withMessage(
        "La ville est requis et doit contenir au moins 4 caractères.",
      )
      .run(req);
  }
  if (req.body.country) {
    ...
    await body("country")
      .isLength({ min: 2 })
      .withMessage("Le pays est requis et doit contenir au moins 2 caractères.")
      .run(req);
  }
  if (req.body.gender) {
    // Validation de genre
    await body("gender")
      .isLength({ min: 1 })
      .withMessage(
        "Le genre est requis et doit contenir au moins 1 caractères.",
      )
      .run(req);
  }
  if (req.body.birthdate) {
    await body("birthdate")
      .notEmpty()
      .withMessage("La date est requise et doit être du type date")
      .run(req);
  }
...
async function validateRegisterUserFields(req) {
  await Promise.all([
    ...
    body("country")
      .optional()
      .isLength({ min: 4 })
      .withMessage("Le pays est requis et doit contenir au moins 4 caractères.")
      .run(req),
    ...
    body("gender")
      ...
      .isLength({ min: 4 })
      .withMessage(
        "Le genre est requis et doit contenir au moins 4 caractères.",
      )
      .run(req),
...
async function validateLicenseData(req) {
  await Promise.all([
    body("number")
      .isLength({ min: 8 })
      .withMessage("Le numéro de licence doit contenir au moins 8 caractères")
      .run(req),
    body("name")
      .optional()
      .isLength({ min: 3 })
    ...
    body("lastName")
      .optional()
      .isLength({ min: 3 })
      .withMessage("Le nom de famille doit contenir au moins 3 caractères")
      .run(req),
    body("birthdate")
      .optional()
      .isString()
    ...
    body("address")
      .optional()
      .isLength({ min: 4 })
      .withMessage("L'adresse doit contenir au moins 4 caractères")
      .run(req),
    body("appartment")
      .optional()
      .isLength({ min: 2 })
      .withMessage("L'appartement doit contenir au moins deux caractères")
      .run(req),
    body("province")
      .optional()
      .isLength({ min: 2 })
      .withMessage("La province doit contenir au moins deux caractères")
      .run(req),
    body("postalCode")
      .optional()
      .isLength({ min: 4 })
      .withMessage("Le code postal doit contenir au moins 4 caractères")
      .run(req),
    body("licenseClass")
      .isLength({ min: 1 })
      .withMessage("La classe de licence doit contenir au moins 1 caractère")
      .run(req),
    // body('sex').isLength({ min: 1 }).withMessage('Le sexe doit contenir au moins 1 caractère').run(req),
    body("sex")
      .optional()
      .isLength({ min: 1 })
      .withMessage("Le sexe doit contenir au moins 1 caractère")
      .custom((value) => {
    ...
    body("rest")
      .optional()
      .isLength({ min: 2 })
      .withMessage('Le champ "rest" doit contenir au moins 2 caractères')
      .run(req),
    body("mention")
      .optional()
      .isLength({ min: 2 })
      .withMessage('Le champ "mention" doit contenir au moins 2 caractères')
      .run(req),
    body("referenceNumber")
      .optional()
      .optional()
      .isLength({ min: 4 })
      .withMessage("Le numéro de référence doit contenir au moins 4 caractères")
      .run(req),
    body("height")
      .optional()
      .isLength({ min: 2 })
      .withMessage("La hauteur doit contenir au moins 2 caractères")
      .run(req),
    body("weight")
      .optional()
      .isLength({ min: 2 })
      .withMessage("Le poids doit contenir au moins 2 caractères")
      .run(req),
    body("issued")
      .isString()
      .matches(/^\d{4}\/\d{2}\/\d{2}$/)
      .withMessage("La date d'émission doit être au format yyyy/mm/dd")
      .run(req),
    ...
    body("expires")
      .isString()
      .matches(/^\d{4}\/\d{2}\/\d{2}$/)
      .withMessage("La date d'expiration doit être au format yyyy/mm/dd")
      .custom((value, { req }) => {
        const expirationDate = new Date(value);
        const currentDate = new Date();
        if (expirationDate <= currentDate) {
          throw new Error(
            "La date d'expiration est déjà passée ou expire aujourd'hui",
          );
        }
    ...
    body("city")
      ...
      .isString()
      .isLength({ min: 4 })
      .withMessage("La ville doit contenir au moins 4 caractères")
      .run(req),
    body("country")
      .optional()
      .isString()
      .isLength({ min: 4 })
      .withMessage("Le pays doit contenir au moins 4 caractères")
      .run(req),
  ]);
}
...
async function RegisterUserSendCode(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({ msg: "Erreur interne du serveur", error: error.message });
  }
}
  ...
  try {
    ...
    if (
      ...
      if (user.verificationAttempts > 3) {
        ...
        return res
          .status(429)
          .json({ msg: "Accès protégé, contacter un administrateur" });
      }

      return res
        .status(400)
        .json({
          msg: "Code de vérification invalide ou expiré. Veuillez réessayer.",
        });
    }
...
async function Login(req, res) {
  try {
    ...
    if (loggedInUser.active) {
      ...
      if (passwordMatch) {
        ...
        if (updatedUser.loginAttempts >= 3) {
          ...
          return res
            .status(401)
            .json({
              msg: "Compte bloqué suite à plusieurs tentatives infructueuses",
            });
        } else {
      ...
      res
        .status(401)
        .json({ msg: "Compte inactif, contactez l'administrateur." });
    }
  } catch (error) {
    console.error(error);
    return res
    ...
    if (loggedInUser.active) {
      ...
      if (!loggedInUser.allFieldsComplete) {
        return res
          .status(403)
          .json({
            msg: "L'utilisateur n'a pas complété toute son inscription.",
          });
      }
      ...
      return res
        .status(200)
        .json({ msg: "Utilisateur authentifié avec succès", user: userInfo });
    } else {
      return res
        .status(401)
        .json({ msg: "Compte inactif, contactez l'administrateur." });
  ...
  try {
    ...
    if (!loggedInUser || loggedInUser.active === false) {
      return res
        .status(403)
        .json({
          msg: "Utilisateur présentant des problèmes avec le compte, contactez l'administrateur",
        });
    }
    ...
    res
      .status(200)
      .json({
        msg: "Session rafraîchie avec succès ",
        A7: refreshToken,
...
async function GetUserById(req, res) {
  try {
    ...
    if (ownerId !== userId) {
      return res
        .status(400)
        .json({ msg: "Impossible d'afficher cette utilisateur" });
    }
...
async function RestorePassword(req, res) {
  try {
    ...
    if (!passwordMatch) {
      return res
        .status(403)
        .json({ msg: "L'ancien mot de passe est incorrect" });
    }
    ...
    return res
      .status(200)
      .json({ msg: "Le mot de passe a été mis à jour avec succès" });
  } catch (error) {
    console.error(
  ...
  try {
    ...
    if (req.body.password) {
      return res
        .status(403)
        .json({
          msg: "La modification du mot de passe n'est pas autorisée depuis cette route",
        });
    }
    // Validation des champs de la requête
...
async function SendVerificationCode(req, res) {
  try {
    ...
    if (!user || !user.active) {
      return res
        .status(404)
        .json({ msg: "Impossible d'exécuter cette action" });
    }
    ...
    return res
      .status(200)
      .json({
        msg: "Un code de vérification a été envoyé à votre adresse électronique.",
        code: verificationCode,
      });
  } catch (error) {
    console.error(
      `Erreur lors de la récupération de l'utilisateur ou lors de l'envoi du code de vérification : ${error.message}`,
    );
    return res
      .status(500)
      .json({ msg: "Erreur interne du serveur", error: error });
...
async function verifyAndChangePassword(req, res) {
  try {
    ...
    if (!newPassword) {
      return res
        .status(400)
        .json({ msg: "Veuillez introduire votre nouveau mot de passe" });
    }
    ...
    if (!user || !user.active) {
      return res
        .status(404)
        .json({ msg: "Utilisateur non trouvé ou compte non actif" });
    }
    ...
    if (
      ...
      return res
        .status(403)
        .json({
          msg: "Code de vérification incorrect ou expiré. Veuillez réessayer.",
        });
    }
    ...
    return res
      .status(200)
      .json({ msg: "Le mot de passe a été modifié avec succès." });
  } catch (error) {
    console.error(
      `Erreur lors de la vérification du code de vérification ou du changement de mot de passe : ${error.message}`,
    );
    return res
      .status(500)
      .json({ msg: "Erreur interne du serveur", error: error });
  ...
  try {
    ...
    if (!documentText || documentText === "Incompatible_format") {
      return res
        .status(400)
        .json({
          msg: "Impossible de lire le document, vérifiez le format et la qualité de l'image.",
        });
    }
    ...
    return res
      .status(200)
      .json({
        msg: "Voici le document",
        document: documentText,
        extractedInfoText: extractedInfo,
      });

    // return res.status(200).json({ msg: 'Hello from yupload document ' });
  } catch (error) {
    console.error(
      `UploadDocument: Erreur interne du serveur : ${error.message}`,
    );
    return res
      .status(500)
      .json({ msg: "Erreur interne du serveur", error: error });
  ...
  try {
    ...
    if (!user || user.driverLicense === "pending") {
      return res
        .status(400)
        .json({
          msg: "Veuillez mettre à jour les détails de votre permis de conduire.",
        });
    }
    ...
    if (!vehicle || !user.vehicles.includes(vehicleId)) {
      return res
        .status(400)
        .json({ msg: "Vérifier les véhicules de l'utilisateur." });
    }
...
function extractIds(dataString) {
  ...
  if (matches && matches.length === 3) {
    ...
    throw new Error(
      "Les identifiants n'ont pas pu être extraits de la chaîne fournie.",
    );
  }
}
  ...
  try {
    ...
    if (alphaNum) {
      ...
      if (!user) {
        return res
          .status(404)
          .json({ msg: "Le code alphanumérique ne fonctionne pas" });
      }

      if (user.alphaNumCode == "non" || user.findMyVehicle == "non") {
        return res
          .status(404)
          .json({ msg: "Demander un nouveau code alphanumérique" });
      }

      let { response, statusCode, msg } = await getUserInfo(
        user._id,
        user.findMyVehicle,
      );
      ...
      if (!id || !iv) {
        return res
          .status(400)
          .json({ msg: "Veuillez compléter tous les champs pertinents" });
      }
    ...
    return res
      .status(500)
      .json({ msg: "Erreur de serveur interne", error: error });
  }
}
...
async function getMyAutoFullInfo(req, res) {
  try {
    ...
    const { response, statusCode, msg } = await getUserInfo(
      myToken.user_id,
      vehicleId,
    );
...
async function encryptMyData(req, res) {
  ...
  const decryptedDataFull = decryptDataAES(
    encryptedDataFull.ed,
    encryptedDataFull.iv,
  );
  ...
  try {
    ...
    if (missingUserFields.length > 0) {
      return res
        .status(400)
        .json({
          msg: "User: Les champs suivants sont manquants",
          name: myUser.name,
    ...
    const vehiclesWithMissingFields = vehicles
      .map((vehicle) => ({
    ...
    const [insurances, drivingLicense] = await Promise.all([
      insurancesPromise,
    ...
    if (vehiclesWithoutInsurance.length > 0) {
      ...
      return res
        .status(400)
        .json({
          msg: `Véhicules sans assurance : (${vehiclesWithoutInsurance.length})`,
          vehiclesWithoutInsurance,
        });
    }
    ...
    const invalidInsurances = insurances.filter(
      (insurance) =>
        findMissingFields(insurance, insuranceFieldsToCheck).length > 0,
    );
    ...
    if (invalidInsurances.length > 0) {
      return res
        .status(400)
        .json({
          msg: "Les champs suivants manquent dans l'assurance",
          invalidInsurances,
        });
    }
    /* DRIVING LICENSE */
    ...
    const missingDrivingLicenseFields = findMissingFields(
      drivingLicense,
      drivingLicenseFieldsToCheck,
    );
    ...
    if (missingDrivingLicenseFields.length > 0) {
      return res
        .status(400)
        .json({
          msg: "Il manque les champs suivants sur le permis de conduire",
          missingDrivingLicenseFields,
        });
    }
    ...
    const response = {
      ...
      vehicles: vehicles.map((vehicle) => ({
        ...
        insurance:
          insurances.find(
            (insurance) => insurance.vehicle === vehicle._id.toString(),
          ) || null,
      })),
    ...
    return res
      .status(500)
      .json({ msg: "Erreur de serveur interne", error: error.message });
  }
}
...
async function getUserSystemInfo(user) {
  try {
    ...
    const [vehicles, insurances, drivingLicense] = await Promise.all([
      vehiclesPromise,
      insurancesPromise,
      drivingLicensePromise,
    ]);
...
async function UploadUserProfileImage(req, res) {
  try {
    ...
    if (!uploadedImage || !uploadedImage.path) {
      return res
        .status(400)
        .json({ msg: "Le fichier téléchargé est invalide." });
    }
    ...
    return res
      .status(500)
      .json({
        msg: "Erreur lors du téléchargement de l'image de profil",
        error: error.message,
      });
  }
}
  ...
  try {
    ...
    if (!selfie || !front || !back) {
      return res
        .status(400)
        .json({
          msg: "Tous les fichiers (selfie, front, back) doivent être téléchargés.",
        });
    }
    ...
    console.error(
      "Erreur lors du téléchargement des photos de la carte d'identité:",
      error,
    );
    return res
      .status(500)
      .json({
        msg: "Erreur lors du téléchargement des photos de la carte d'identité",
        error: error.message,
      });
  }
}
