...
async function webHookEvents(req, res) {
    try {
        ...
        switch (code) {
            case 7001:  // Actions en cas d'approbation de la vérification
                console.log('Action started :', id);
                myResponse = await modifAndGetUserVeriffAttributes(id, 'no-action', 'started', true);
            ...
            case 7002: // Actions en cas de rejet de la vérification
                console.log('Action submitted :', id);
                myResponse = await modifAndGetUserVeriffAttributes(id, 'no-action', 'submitted', false);
